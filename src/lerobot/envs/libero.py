...
class LiberoEnv(gym.Env):
    ...
    def step(self, action: np.ndarray) -> tuple[RobotObservation, float, bool, bool, dict[str, Any]]:
        ...
        # bddl_base_domain.step() overrides the returned `done` with _check_success(),
        # dropping the horizon-termination signal.  Robosuite base still sets
        # self.done=True when timestep >= horizon, which causes the next step() call
        # to raise ValueError("executing action in terminated episode").
        # Read the flag directly from the underlying robosuite env to catch this case.
        horizon_done = bool(getattr(self._env.env, "done", False))
        terminated = done or is_success or horizon_done
        info.update(
            {
                "task": self.task,
