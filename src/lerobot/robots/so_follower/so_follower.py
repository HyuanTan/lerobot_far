...
# Maps Feetech register name → observation key suffix used in get_observation / _motors_ft.
# Extra registers recorded per-motor when record_motor_state is enabled: {register_name: obs_key_suffix}
_STATE_FIELD_SUFFIX: dict[str, str] = {
    "Present_Load":    "load",
    "Present_Current": "current",
}

# # Covers addresses 56–63 (contiguous block) plus Present_Current (addr 69, separate read).
# _STATE_FIELD_SUFFIX: dict[str, str] = {
...
class SOFollower(Robot):
    ...
    def _motors_ft(self) -> dict[str, type]:
        ft: dict[str, type] = {f"{motor}.pos": float for motor in self.bus.motors}
        for motor in self.config.record_motor_state:
            for suffix in _STATE_FIELD_SUFFIX.values():
                ft[f"{motor}.{suffix}"] = float
        return ft
    ...
    def get_observation(self) -> RobotObservation:
        ...
        obs_dict: dict = {f"{motor}.pos": val for motor, val in raw.items()}

        if self.config.record_motor_state:
            for reg, suffix in _STATE_FIELD_SUFFIX.items():
                vals = self.bus.sync_read(reg, self.config.record_motor_state)
