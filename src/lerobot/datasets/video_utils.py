...
def encode_video_frames(
    ...
    with av.open(str(video_path), "w") as output:
        # PyAV's to_avrational requires a value with .numerator (int or Fraction, not float).
        output_stream = output.add_stream(vcodec, Fraction(fps).limit_denominator(1_000_000), options=video_options)
        ...
        try:
            ...
            while True:
                ...
                if container is None:
                    ...
                    _fps_frac = Fraction(self.fps).limit_denominator(1_000_000)
                    output_stream = container.add_stream(self.vcodec, _fps_frac, options=video_options)
                    output_stream.pix_fmt = self.pix_fmt
                    output_stream.width = width
                    output_stream.height = height
                    output_stream.time_base = Fraction(1, int(_fps_frac))

                # Encode frame with explicit timestamps
                pil_img = Image.fromarray(frame_data)
                video_frame = av.VideoFrame.from_image(pil_img)
                video_frame.pts = frame_count
                video_frame.time_base = Fraction(1, int(_fps_frac))
