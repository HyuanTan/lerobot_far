...
def get_safe_version(repo_id: str, version: str | packaging.version.Version) -> str:
    ...
    if not hub_versions:
        # RevisionNotFoundError inherits from HfHubHTTPError which now requires
        # a `response` keyword argument in newer huggingface_hub versions.
        # This branch is not an HTTP error — it's a missing version-tag on the repo —
        # so raise EnvironmentError instead to avoid the constructor mismatch.
        raise EnvironmentError(
            f"Dataset '{repo_id}' has no codebase version tags on the Hub.\n"
            f"Assuming _version_ is the codebase_version value in meta/info.json, run:\n"
            f"  from huggingface_hub import HfApi\n"
            f"  HfApi().create_tag('{repo_id}', tag='_version_', repo_type='dataset')"
        )
