...
class RecordConfig:
    ...
    # Run display logging in a background thread so record loop never blocks on visualization
    display_async: bool = True
    # Minimum interval between two image updates sent to Rerun (seconds)
    display_image_interval_s: float = 0.2
    # How often the background display worker wakes up when idle
    display_worker_poll_interval_s: float = 0.01
...
class _ScalarDisplayPayload:
    observation: RobotObservation | None
    action: RobotAction | None


def _split_observation_for_display(
    observation: RobotObservation | None,
) -> tuple[RobotObservation | None, RobotObservation | None]:
    """Split observation into scalar-like (state) and image-like parts for separate display paths."""
    ...
    return (
        scalar_like_obs if scalar_like_obs else None,
        image_like_obs if image_like_obs else None,
    )


class AsyncRerunDisplayWorker:
    """
    Background Rerun logger:
    - action/state: enqueue every step (small payload, no downsampling)
    - images: latest-only, downsampled by time interval

    Main loop never blocks on display.
    """

    def __init__(
        self,
        compress_images: bool = False,
        image_interval_s: float = 0.2,
        poll_interval_s: float = 0.01,
    ):
        self.compress_images = compress_images
        self.image_interval_s = max(image_interval_s, 1e-6)
        self.poll_interval_s = max(poll_interval_s, 1e-4)

        self._scalar_queue: SimpleQueue[_ScalarDisplayPayload] = SimpleQueue()

        self._latest_image_observation: RobotObservation | None = None
    ...
    def start(self) -> None:
        if self._thread is not None:
            return
        self._thread = Thread(target=self._run, name="rerun-display-worker", daemon=True)
        self._thread.start()
    ...
    def submit(
        ...
        scalar_obs, image_obs = _split_observation_for_display(observation)

        # action/state: every step, no downsampling
        if scalar_obs is not None or action is not None:
            self._scalar_queue.put(_ScalarDisplayPayload(observation=scalar_obs, action=action))

        # image: latest-only
        if image_obs is not None:
            with self._image_lock:
                self._latest_image_observation = image_obs
    ...
    def _drain_scalar_queue(self) -> None:
        while True:
            try:
                payload = self._scalar_queue.get_nowait()
            except Empty:
                break
            log_rerun_data(
    ...
    def _pop_latest_image_observation(self) -> RobotObservation | None:
        with self._image_lock:
            image_obs = self._latest_image_observation
            self._latest_image_observation = None
        return image_obs
    ...
    def _run(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            self._drain_scalar_queue()

            now = time.perf_counter()
            if now - last_image_log_t >= self.image_interval_s:
                image_obs = self._pop_latest_image_observation()
                if image_obs is not None:
                    log_rerun_data(
...
def record_loop(
    ...
    display_compressed_images: bool = False,
    display_worker: AsyncRerunDisplayWorker | None = None,
):
    ...
    while timestamp < control_time_s:
        ...
        if display_data:
            if display_worker is not None:
                # Non-blocking: visualization is best-effort and must not slow recording
                display_worker.submit(
...
def record(cfg: RecordConfig) -> LeRobotDataset:
    ...
    try:
        if cfg.display_data and cfg.display_async:
            display_worker = AsyncRerunDisplayWorker(
                compress_images=display_compressed_images,
                image_interval_s=cfg.display_image_interval_s,
                poll_interval_s=cfg.display_worker_poll_interval_s,
            )
