...
# Contiguous state feedback block: Present_Position (addr 56) through Present_Temperature (addr 63).
# addr 64 is undefined on STS3215, so the block must stop at 63 to avoid Range Error responses.
# Reading this single 8-byte block replaces 5 individual sync_read calls.
_STATE_BLOCK_ADDR = 56
_STATE_BLOCK_LENGTH = 8  # covers addresses 56..63 inclusive (no gaps)

# Fields within the block: {register_name: (address, byte_length)}
_STATE_BLOCK_FIELDS: dict[str, tuple[int, int]] = {
...
class FeetechMotorsBus(SerialMotorsBus):
    ...
    def sync_read_motor_state(
        self,
        motors: "NameOrID | list[NameOrID] | None" = None,
        num_retry: int = 0,
    ) -> dict[str, dict[str, float]]:
        """Read all state feedback fields in a single bus transaction.

        Reads the contiguous register block at addresses 56–70, covering
        Present_Position, Present_Velocity, Present_Load, Present_Voltage,
        Present_Temperature, Status, Moving, and Present_Current.

        Returns:
            dict mapping motor name → {field_name: value}. Present_Position is
            calibration-normalized; sign-magnitude fields are decoded.
        """
        self._assert_protocol_is_compatible("sync_read")
        names = self._get_motors_list(motors)
        ids = [self.motors[name].id for name in names]

        self._setup_sync_reader(ids, _STATE_BLOCK_ADDR, _STATE_BLOCK_LENGTH)
        for n_try in range(1 + num_retry):
            comm = self.sync_reader.txRxPacket()
            if self._is_comm_success(comm):
                break
            logger.debug(
                f"Failed to sync read motor state block ({n_try=}): "
                + self.packet_handler.getTxRxResult(comm)
            )

        if not self._is_comm_success(comm):
            raise ConnectionError(
                f"Failed to sync read motor state block: {self.packet_handler.getTxRxResult(comm)}"
            )

        result: dict[str, dict[str, float]] = {}
        for name, id_ in zip(names, ids):
            fields: dict[str, float] = {}
            for data_name, (addr, length) in _STATE_BLOCK_FIELDS.items():
                raw = self.sync_reader.getData(id_, addr, length)
                decoded = self._decode_sign(data_name, {id_: raw})[id_]
                if data_name in self.normalized_data:
                    fields[data_name] = float(self._normalize({id_: decoded})[id_])
                else:
                    fields[data_name] = float(decoded)
            result[name] = fields

        # Present_Current (addr 69) is not contiguous with the block above (gap at 64, 67-68),
        # so it requires a separate bus transaction.
        current_values = self.sync_read("Present_Current", motors, normalize=False)
        for name, val in current_values.items():
            result[name]["Present_Current"] = float(val)
