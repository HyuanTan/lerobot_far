#!/usr/bin/env bash
# eval-scripts/sync_libero_smolvla_eval-nortc.sh
#
# Sweep: env_task × inference_latency × actions_per_chunk
# Mode : synchronous inference (chunk_size_threshold=0), no RTC
#
# ── Per-combo independent design ──────────────────────────────────────────────
#   Server restarts for every (suite, d, K) combo.
#   All data live in the same eval_dir — no shared server directory, no symlinks.
#
#   eval_dir = outputs/eval_thesis/libero/<suite>/sync_nortc/smolvla/latency_s<d>/K<K>/
...
#
# ── Skip condition ────────────────────────────────────────────────────────────
#   K < d: inference delay spans entire execution window → robot always stalls.
#
# ── Usage ─────────────────────────────────────────────────────────────────────
#   bash eval-scripts/sync_libero_smolvla_eval-nortc.sh
#   SKIP_EXISTING=true bash eval-scripts/sync_libero_smolvla_eval-nortc.sh
#   SAVE_VIDEO=true    bash eval-scripts/sync_libero_smolvla_eval-nortc.sh
...
ACTIONS_PER_CHUNK_LIST=(1 5 10 20 30 40)

SUITES=(libero_object)
# INFERENCE_LATENCIES_STEPS=(0 6)
# ACTIONS_PER_CHUNK_LIST=(6 10)
...
# ══ Summary counters ══════════════════════════════════════════════════════════
total_combos=$(( ${#SUITES[@]} * ${#INFERENCE_LATENCIES_STEPS[@]} * ${#ACTIONS_PER_CHUNK_LIST[@]} ))
done_combos=0
skipped_combos=0
failed_combos=0

echo "════════════════════════════════════════════════════════════════════"
echo "  eval-scripts/sync_libero_smolvla_eval-nortc.sh"
echo "  method            : ${METHOD}"
...
echo "  actions_per_chunk : ${ACTIONS_PER_CHUNK_LIST[*]}"
echo "  suites            : ${SUITES[*]}"
echo "  total combos      : ${total_combos}  (before K<d skips)"
...
# ══ Main sweep ════════════════════════════════════════════════════════════════
# Loop: d → K → suite
# Server starts and stops once per (d, K, suite).
...
for delay_steps in "${INFERENCE_LATENCIES_STEPS[@]}"; do
    ...
    for actions_per_chunk in "${ACTIONS_PER_CHUNK_LIST[@]}"; do

        # Skip K < d: inference delay spans entire execution window → robot stalls.
        if (( actions_per_chunk < delay_steps )); then
            echo "[skip] K=${actions_per_chunk} < d=${delay_steps} steps — degenerate combo, skipping"
        ...
        for env_task in "${SUITES[@]}"; do
            ...
            log_root="./outputs/eval_thesis/${benchmark_robot_type}/${env_task}/${METHOD}/${model_type}/latency_${latency_tag}/K${actions_per_chunk}"
            combo_label="suite=${env_task}  d=${delay_steps}steps(${latency}s)  K=${actions_per_chunk}"
            ...
            stdbuf -oL -eL uv run python -m lerobot.async_inference.sim_test.run_libero_test \
                --config_path "src/lerobot/async_inference/config/${benchmark_robot_type}/sync_client.yaml" \
                --env_task="${env_task}" \
