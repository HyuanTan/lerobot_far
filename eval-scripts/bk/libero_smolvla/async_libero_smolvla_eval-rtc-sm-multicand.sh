#!/usr/bin/env bash
# eval-scripts/async_libero_smolvla_eval-rtc-sm-multicand.sh
#
# Sweep: env_task × inference_latency × rtc_execution_horizon
# Mode : asynchronous inference with RTC (SM multi-candidate variant)
#
...
#
#   eval_dir = outputs/eval_thesis/libero/<suite>/async_rtc_sm_multicand/smolvla/latency_s<d>/H<H>/
...
#
# ── Usage ─────────────────────────────────────────────────────────────────────
#   bash eval-scripts/async_libero_smolvla_eval-rtc-sm-multicand.sh
#   SKIP_EXISTING=true bash eval-scripts/async_libero_smolvla_eval-rtc-sm-multicand.sh
#   SAVE_VIDEO=true    bash eval-scripts/async_libero_smolvla_eval-rtc-sm-multicand.sh
...
echo "════════════════════════════════════════════════════════════════════"
echo "  eval-scripts/async_libero_smolvla_eval-rtc-sm-multicand.sh"
echo "  method              : ${METHOD}"
    ...
    for rtc_horizon in "${RTC_HORIZON_LIST[@]}"; do
        ...
        for env_task in "${SUITES[@]}"; do
            ...
            stdbuf -oL -eL uv run python -m lerobot.async_inference.policy_server \
                --config_path "src/lerobot/async_inference/config/${benchmark_robot_type}/async_smolvla_server_sm.yaml" \
                --host="${SERVER_HOST}" \
                --port="${SERVER_PORT}" \
