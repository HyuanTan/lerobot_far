#!/usr/bin/env bash
# eval-scripts/so101_smolvla/async_so101_smolvla_client-rtc-sm-multican.sh
#
# Robot client for SO-101 smolvla multi-candidate SM eval — runs on the robot machine.
# Transmits the model identity to the server on connect; server loads the model.
#
# ── Workflow ─────────────────────────────────────────────────────────────────
#   1. Start async_so101_smolvla_server-rtc-sm-multican.sh on the GPU machine first.
...
#
# ── Usage ─────────────────────────────────────────────────────────────────────
#   bash eval-scripts/so101_smolvla/async_so101_smolvla_client-rtc-sm-multican.sh
#   SERVER_IP=192.168.1.100 bash eval-scripts/so101_smolvla/async_so101_smolvla_client-rtc-sm-multican.sh
...
# ══ RTC settings ══════════════════════════════════════════════════════════════
RTC_HORIZON=20           # H (steps): rtc_execution_horizon
...
echo "════════════════════════════════════════════════════════════════════"
echo "  SO-101 smolvla — multi-candidate SM robot client"
echo "  method  : ${METHOD}"
...
stdbuf -oL -eL python -m lerobot.async_inference.run_so101_multicand_client \
    --config_path "src/lerobot/async_inference/config/${benchmark_robot_type}/async_client_sm_multican.yaml" \
    --task="${TASK}" \
