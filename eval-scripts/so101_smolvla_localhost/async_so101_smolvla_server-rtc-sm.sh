#!/usr/bin/env bash
# eval-scripts/so101_smolvla/async_so101_smolvla_server-rtc-sm.sh
#
# Policy server for SO-101 smolvla (SM / SM+inter) — runs on the GPU machine.
...
#   2. Run async_so101_smolvla_client-rtc-sm.sh on the robot machine.
#   3. Stop this server manually with Ctrl-C when done.
#
# ── Usage ─────────────────────────────────────────────────────────────────────
#   bash eval-scripts/so101_smolvla/async_so101_smolvla_server-rtc-sm.sh
#   CUDA_VISIBLE_DEVICES=0 bash eval-scripts/so101_smolvla/async_so101_smolvla_server-rtc-sm.sh
...
# ══ Method tag ════════════════════════════════════════════════════════════════
METHOD=async_rtc_sm # async_rtc_sm_inter
...
echo "════════════════════════════════════════════════════════════════════"
echo "  SO-101 smolvla — SM policy server"
echo "  method      : ${METHOD}"
