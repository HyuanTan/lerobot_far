#!/usr/bin/env bash
# =============================================================================
# eval_pi05.sh  —  Pi05 / SmolVLA  LIBERO sweep
#
...
#
# Usage:
#   bash eval_pi05.sh [--dry-run] [--delay-only] [--horizon-only] [--no-sm]
#
...
# eval_pi05.sh may be called from any cwd; cd to project root so that
...
# ── Configuration ─────────────────────────────────────────────────────────────
CKPT="${CKPT:-lerobot/pi05_libero_finetuned_v044}"
...
OUT_ROOT="${OUT_ROOT:-outputs/eval_thesis_sim/libero/pi05}"
...
# Override via env: FORCE=1 bash eval_pi05.sh, or pass --force flag.
FORCE="${FORCE:-0}"
...
for arg in "$@"; do
  case "$arg" in
    ...
    --force)          FORCE=1 ;;
    --no-force)       FORCE=0 ;;
    # Legacy SM flags (kept for backward compat)
    --no-sm)          METHODS="${METHODS:-baseline rtc}" ;;
    ...
    --sm-only)        METHODS="baseline_sm rtc_sm" ;;
  esac
done
